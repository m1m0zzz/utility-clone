...
class CustomLookAndFeel : public juce::LookAndFeel_V4 {
 public:
  ...
  void drawLabel(juce::Graphics& g, juce::Label& label) override {
    ...
    const auto rect =
        juce::Rectangle(0, 0, label.getWidth(), label.getHeight());
    const auto editor = label.getCurrentTextEditor();
    if (editor != nullptr) {
      editor->applyColourToAllText(
          label.findColour(juce::TextEditor::ColourIds::textColourId), true);
  ...
  juce::Label* createSliderTextBox(juce::Slider& slider) override {
    ...
    label->setColour(juce::Label::ColourIds::outlineColourId,
                     juce::Colours::transparentWhite);
    label->setColour(juce::Label::ColourIds::outlineWhenEditingColourId,
                     themeColours.at("text"));
    label->setColour(juce::TextEditor::ColourIds::textColourId,
                     themeColours.at("text"));
    label->setColour(juce::TextEditor::ColourIds::backgroundColourId,
                     themeColours.at("white"));
    label->setColour(juce::TextEditor::ColourIds::outlineColourId,
                     themeColours.at("lightblack"));
    label->setColour(juce::TextEditor::ColourIds::highlightedTextColourId,
                     themeColours.at("text"));
    label->setColour(juce::TextEditor::ColourIds::highlightColourId,
                     juce::Colours::transparentWhite);
    return label;
  };
  ...
  juce::Slider::SliderLayout getSliderLayout(juce::Slider& slider) override {
    // ref:
    // https://github.com/juce-framework/JUCE/blob/master/modules/juce_gui_basics/lookandfeel/juce_LookAndFeel_V2.cpp#L1651
    ...
    auto textBoxWidth = std::max(
        0,
        std::min(slider.getTextBoxWidth(), localBounds.getWidth() - minXSpace));
    textBoxWidth =
        static_cast<int>(textBoxWidth * 0.85);  // custamized this line
    auto textBoxHeight =
        std::max(0, std::min(slider.getTextBoxHeight(),
  ...
  void drawButtonBackground(juce::Graphics& g, juce::Button& button,
    ...
    auto bgOn =
        button.findColour(juce::TextButton::ColourIds::buttonOnColourId);
    auto outline =
        button.findColour(juce::ComboBox::ColourIds::outlineColourId);
