 ...
 public:
  ...
  void drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height,
    ...
    valueArc.addCentredArc(bounds.getCentreX(), bounds.getCentreY(), arcRadius, arcRadius, 0.0f,
                           2.0f * static_cast<float>(M_PI), toAngle, true);
    g.setColour(fill);
    ...
    p.applyTransform(juce::AffineTransform::rotation(toAngle).translated(bounds.getCentreX(),
                                                                         bounds.getCentreY()));
    g.setColour(thumb);
    g.fillPath(p);
  };
  ...
  void drawLabel(juce::Graphics& g, juce::Label& label) override {
    ...
    if (editor != nullptr) {
      editor->applyColourToAllText(label.findColour(juce::TextEditor::ColourIds::textColourId),
                                   true);
  ...
  juce::Slider::SliderLayout getSliderLayout(juce::Slider& slider) override {
    ...
    auto textBoxWidth =
        std::max(0, std::min(slider.getTextBoxWidth(), localBounds.getWidth() - minXSpace));
    textBoxWidth = static_cast<int>(textBoxWidth * 0.85);  // custamized this line
    auto textBoxHeight =
        std::max(0, std::min(slider.getTextBoxHeight(), localBounds.getHeight() - minYSpace));
